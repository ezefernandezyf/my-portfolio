...
describe('ProjectsPage coverage', () => {
  it(
    'filters projects, loads more items, and preserves missing action branches',
    { timeout: 15000 },
    async () => {
      ...
      expect(
        Array.from(betaLinks).some((link) => link.getAttribute('href')?.includes('beta-demo')),
      ).toBe(true);
      expect(
        Array.from(betaLinks).some((link) => link.getAttribute('href')?.includes('beta-repo')),
      ).toBe(false);
      ...
      expect(
        screen.queryByRole('button', { name: /load more|cargar más/i }),
