...
5. JSON-LD `@graph` schema (Person, WebSite, WebPage, Project, Article) per route
6. Per-route unique meta tags in prerendered HTML (title, description, OG, Twitter, canonical)
7. `llms.txt` + `/.well-known/llms.txt`
8. MetaTags component adaptation (detect prerendered tags, avoid duplicates)
9. Sitemap expansion: all 11 routes with lastmod + changefreq

**Slice C — Design Fixes + Polish (~200 lines)**
10. Fix PrivacyPage CSS tokens (replace undefined tokens with `@theme` tokens from index.css)
11. Fix CaseStudyTemplate CSS tokens (same)
12. Sitemap submission verification
