...
(Previously: Titles stored as i18n keys under `meta.title`, resolved via `useTranslation`.)
...
(Previously: Descriptions stored under `meta.description` i18n keys.)
...
(Previously: Coverage via i18n namespace JSON with `meta.title`/`meta.description`.)
